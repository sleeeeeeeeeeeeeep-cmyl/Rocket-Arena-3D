...
function dressCar(wrap, size, min, fit) {
  ...
  // shrink-wrapped glass: a heightfield shell sampled from THIS car's real roof surface,
  // hovering a hair above it so it reads as the car's own glass, not a dome.
  let glass;
  if (fit && fit.shell) {
    const sh = fit.shell, nx = sh.nx, nz = sh.nz;
    const pos = [], idx = [];
    for (let iz = 0; iz < nz; iz++) for (let ix = 0; ix < nx; ix++) {
      const gx = -sh.w + (ix / (nx - 1)) * sh.w * 2;
      const gz = sh.z - sh.l + (iz / (nz - 1)) * sh.l * 2;
      // edge vertices pull down to hug the body; interior floats slightly above the roof
      const edge = (ix === 0 || ix === nx - 1 || iz === 0 || iz === nz - 1) ? -H * 0.015 : H * 0.03;
      pos.push(gx, sh.grid[iz * nx + ix] + edge, gz);
    }
    ...
    glass = new THREE.Mesh(shellGeo, glassMat);
  } else {
    glass = new THREE.Mesh(new THREE.SphereGeometry(1, 24, 14, 0, Math.PI * 2, 0, Math.PI * 0.55), glassMat);
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    housing.rotation.x = Math.PI / 2; head.add(housing);
    const lens = new THREE.Mesh(new THREE.CircleGeometry(lampR * 0.76, 16), lensMat);
    lens.position.z = -0.075; lens.rotation.y = Math.PI; head.add(lens);
    ...
    bl.position.z = 0.045; tail.add(bl);
