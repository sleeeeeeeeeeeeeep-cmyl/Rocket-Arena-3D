...
function normalizeModel(root, targetLength, faceFix) {
  ...
  root.traverse(o => { if (o.isMesh && o.material) { o.castShadow = true;
    // The imported print models carry a noisy baked texture atlas that looks chaotic
    // and makes colour-changing impossible (tint multiplies over the texture).
    // Replace it with a clean painted-metal material so the body reads as one solid colour.
    const clean = new THREE.MeshStandardMaterial({
      color: 0xcfd6e0,        // default light steel; overridden by styleCar/team colour
      metalness: 0.35,
      roughness: 0.42,
      envMapIntensity: 1.3
    });
