...
function dressCar(wrap, size, min, fit) {
  ...
  const wr = (fit && fit.wheelR) ? fit.wheelR : H * 0.30, ww = W * 0.082;
  ...
  const tireGeo = new THREE.TorusGeometry(wr * 0.74, wr * 0.25, 12, 26);
  ...
  // CONFORMING WINDSHIELD: a glass strip laid ON the model's own windshield slope
  // (sampled from the real surface between the hood and the cabin peak), so it takes
  // the exact shape of each car's sculpted glass instead of approximating it.
  ...
  (function buildWindows() {
    ...
    const halfWglass = Math.min(cabW * 0.44, W * 0.42);
    const zFront = cabZ - Math.max(L * 0.10, cabLen * 0.85);
    const zBack = cabZ + cabLen * 0.15;
    const cols = 9, rows = 6, pos = [], idx = [];
    for (let r = 0; r <= rows; r++) {
      const z = zFront + (zBack - zFront) * (r / rows);
      for (let ci = 0; ci <= cols; ci++) {
        const u = ci / cols - 0.5;
        const x = u * 2 * halfWglass;
        const edge = (ci === 0 || ci === cols || r === 0) ? -H * 0.008 : H * 0.02;
        pos.push(x, sampleTop(x, z) + edge, z);
      }
    }
    ...
    fgeo.setIndex(idx); fgeo.computeVertexNormals();
    glassMat.side = THREE.DoubleSide;
    glassGrp.add(new THREE.Mesh(fgeo, glassMat));
    // black trim tracing the glass edge
    const trim = new THREE.LineSegments(new THREE.EdgesGeometry(fgeo, 40), new THREE.LineBasicMaterial({ color: 0x0a0c10 }));
    glassGrp.add(trim);
    // wipers resting at the glass base
    const hoodY = sampleTop(0, zFront);
    const wiperMat = new THREE.MeshStandardMaterial({ color: 0x0c0e12, roughness: 0.7 });
    for (let wp = -1; wp <= 1; wp += 2) {
      const wiper = new THREE.Mesh(new THREE.BoxGeometry(halfWglass * 0.7, H * 0.012, H * 0.02), wiperMat);
      wiper.position.set(wp * halfWglass * 0.38, hoodY + H * 0.025, zFront + L * 0.012);
      wiper.rotation.y = wp * 0.26;
      glassGrp.add(wiper);
    }
    ...
    for (let ms = -1; ms <= 1; ms += 2) {
      ...
      mgrp.position.set(ms * Math.min(cabW * 0.5, W * 0.44), Math.min(cabTop * 0.72, sampleTop(ms * halfWglass, cabZ) + H * 0.02), zFront + L * 0.05);
      glassGrp.add(mgrp);
    }
  })();
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    const bezel = new THREE.Mesh(new THREE.TorusGeometry(lampR, lampR * 0.09, 8, 20),
    ...
    const cover = new THREE.Mesh(new THREE.CircleGeometry(lampR * 1.05, 18),
      new THREE.MeshPhysicalMaterial ? new THREE.MeshPhysicalMaterial({ color: 0xdfe8f5, metalness: 0.1, roughness: 0.03, transparent: true, opacity: 0.14, clearcoat: 1.0, side: THREE.DoubleSide })
    ...
    const amber = new THREE.Mesh(new THREE.BoxGeometry(lampR * 0.34, lampR * 0.2, 0.03),
      new THREE.MeshStandardMaterial({ color: 0xffa020, emissive: 0xff9010, emissiveIntensity: 1.2 }));
    amber.position.set(s * lampR * 0.62, -lampR * 0.5, -0.04); head.add(amber);
