...
function normalizeModel(root, targetLength, faceFix) {
  ...
  const fit = (function sampleFit() {
    ...
    // real wheel centres: the z-clusters where geometry reaches the ground on each half
    const slices = 26, minYs = [];
    for (let s2 = 0; s2 < slices; s2++) {
      const z0 = -L2 / 2 + (s2 + 0.5) * (L2 / slices);
      const sl = pts.filter(p => Math.abs(p[2] - z0) < L2 / slices);
      minYs.push(sl.length ? Math.min(...sl.map(p => p[1])) : 999);
    }
    const lowThresh = size.y * 0.10;
    const centroid = half => {
      let wsum = 0, zsum = 0;
      for (let s2 = 0; s2 < slices; s2++) {
        const z0 = -L2 / 2 + (s2 + 0.5) * (L2 / slices);
        if ((half < 0 ? z0 < 0 : z0 > 0) && minYs[s2] < lowThresh) { const w2 = lowThresh - minYs[s2] + 0.001; wsum += w2; zsum += z0 * w2; }
      }
      return wsum > 0 ? zsum / wsum : half * L2 * 0.34;
    };
    const wheelZF = Math.max(-L2 * 0.46, Math.min(-L2 * 0.18, centroid(-1)));
    const wheelZR = Math.min(L2 * 0.46, Math.max(L2 * 0.18, centroid(1)));
    const fenderAt = z => {
      const zone = pts.filter(p => Math.abs(p[2] - z) < L2 * 0.09 && p[1] < size.y * 0.55);
      return zone.length ? Math.max(...zone.map(p => Math.abs(p[0]))) : size.x * 0.46;
    };
    const fenderF = fenderAt(wheelZF), fenderR = fenderAt(wheelZR);
    // real wheel radius: height of the outboard geometry in the wheel zones
    const wheelRad = (z, fx2) => {
      const zone = pts.filter(p => Math.abs(p[2] - z) < L2 * 0.08 && Math.abs(p[0]) > fx2 * 0.7);
      if (!zone.length) return size.y * 0.30;
      return Math.max(size.y * 0.20, Math.min(size.y * 0.38, Math.max(...zone.map(p => p[1])) * 0.52));
    };
    const wheelR = (wheelRad(wheelZF, fenderF) + wheelRad(wheelZR, fenderR)) / 2;
    ...
    return { noseY, noseSpread, noseH, noseSurfZ, cabinZ, cabinTop, cabinW, cabinLen, lampZs, tailZs, taper, shell, fenderF, fenderR, wheelZF, wheelZR, wheelR, top };
  })();
...
function dressCar(wrap, size, min, fit) {
  ...
  const wr = (fit && fit.wheelR) ? fit.wheelR : H * 0.30, ww = W * 0.115;
  ...
  [[-1, -1], [1, -1], [-1, 1], [1, 1]].forEach(c => {
    ...
    // 5 paired, tapered spokes (Y-spoke style) + chrome hubcap + rim face ring
    const spokes = new THREE.Group();
    for (let s = 0; s < 5; s++) {
      const arm = new THREE.Group();
      for (let pair = -1; pair <= 1; pair += 2) {
        const spoke = new THREE.Mesh(spokeGeo, rimMat);
        spoke.scale.set(0.42, 1, 0.7);           // slimmer paired blades
        spoke.position.y = wr * 0.3;
        spoke.rotation.x = pair * 0.16;          // splay the pair apart
        spoke.position.z = pair * wr * 0.05;
        arm.add(spoke);
      }
      arm.rotation.x = s * (Math.PI * 2 / 5);
      spokes.add(arm);
    }
    const hubcap = new THREE.Mesh(new THREE.SphereGeometry(wr * 0.17, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2), rimMat);
    hubcap.rotation.z = -sx * Math.PI / 2; hubcap.position.x = sx * ww * 0.36; spokes.add(hubcap);
    const face = new THREE.Mesh(new THREE.RingGeometry(wr * 0.44, wr * 0.56, 20),
      new THREE.MeshStandardMaterial({ color: 0xd7dbe2, metalness: 0.92, roughness: 0.22, envMapIntensity: 1.5, side: THREE.DoubleSide }));
    face.rotation.y = Math.PI / 2; face.position.x = sx * ww * 0.34; spokes.add(face);
    ...
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(wr * 0.55, wr * 0.55, ww * 0.55, 16, 1, true),
      new THREE.MeshStandardMaterial({ color: 0x14171c, metalness: 0.6, roughness: 0.6, side: THREE.DoubleSide }));
    barrel.rotation.z = Math.PI / 2; barrel.position.x = -sx * ww * 0.05;
    spinner.add(barrel, tire, rotor, dish, lip, hub, spokes);
    ...
    const isFront = sz < 0;   // nose is -Z: THESE are the steering wheels (was wrongly +Z)
    const fx = fit ? (isFront ? fit.fenderF : fit.fenderR) : W * 0.46;
    const wz = fit ? (isFront ? fit.wheelZF : fit.wheelZR) : sz * L * 0.34;
    wheelGroup.position.set(sx * Math.max(W * 0.34, Math.min(W * 0.52, fx * 0.985)), bottom + wr, wz);
    wheelGroup.userData.front = isFront; wheelGroup.userData.spinner = spinner;
    tagPart(wheelGroup);
  ...
  (function buildWindows() {
    ...
    let hoodY = Math.min(roofY * 0.72, sampleTop(0, frontLowZ) + H * 0.02);
    const effRoof = Math.max(roofY * 0.97, hoodY + H * 0.18);   // flat cars still get a visible pane
    const cols = 7, rows = 4, pane = [], pidx = [];
    for (let r = 0; r <= rows; r++) {
      const t = r / rows;
      const z = frontLowZ + (frontTopZ - frontLowZ) * t;
      const yy = hoodY + (effRoof - hoodY) * t;
    ...
    for (let i = 0; i < rp.count; i++) rp.setZ(i, (cabZ + cabHalf * 0.35) + ((cabZ + cabHalf + L * 0.06) - (cabZ + cabHalf * 0.35)) * ((rp.getY(i) - effRoof) / (hoodY - effRoof)) );
    ...
    for (let s = -1; s <= 1; s += 2) {
      ...
      sw.position.set(s * Math.min(cabW * 0.485, W * 0.43), roofY * 0.76, cabZ);
      glassGrp.add(sw);
    }
    glassMat.side = THREE.DoubleSide;
    // side mirrors on short stalks at the A-pillar bases
    const mirMat = new THREE.MeshStandardMaterial({ color: 0x14171c, metalness: 0.5, roughness: 0.4 });
    for (let ms = -1; ms <= 1; ms += 2) {
      const mgrp = new THREE.Group();
      const stalk = new THREE.Mesh(new THREE.BoxGeometry(W * 0.05, H * 0.02, H * 0.02), mirMat);
      stalk.position.x = ms * W * 0.028; mgrp.add(stalk);
      const mhead = new THREE.Mesh(new THREE.BoxGeometry(W * 0.06, H * 0.05, H * 0.09), mirMat);
      mhead.position.x = ms * W * 0.062; mgrp.add(mhead);
      const mface = new THREE.Mesh(new THREE.PlaneGeometry(W * 0.045, H * 0.038),
        new THREE.MeshStandardMaterial({ color: 0xcfe0f0, metalness: 1.0, roughness: 0.05, envMapIntensity: 2.0 }));
      mface.position.set(ms * W * 0.062, 0, H * 0.047); mgrp.add(mface);
      mgrp.position.set(ms * Math.min(cabW * 0.5, W * 0.44), roofY * 0.60, frontLowZ + L * 0.06);
      glassGrp.add(mgrp);
    }
    // black trim seal tracing the windshield edges
    const trimMat = new THREE.LineBasicMaterial({ color: 0x0a0c10, linewidth: 2 });
    glassGrp.children.slice(0, 2).forEach(paneMesh => {
      const edges = new THREE.EdgesGeometry(paneMesh.geometry, 30);
      const trim = new THREE.LineSegments(edges, trimMat);
      trim.position.copy(paneMesh.position); trim.rotation.copy(paneMesh.rotation);
      glassGrp.add(trim);
    });
    // wiper pair resting at the windshield base
    const wiperMat = new THREE.MeshStandardMaterial({ color: 0x0c0e12, roughness: 0.7 });
    for (let wp = -1; wp <= 1; wp += 2) {
      const wiper = new THREE.Mesh(new THREE.BoxGeometry(cabW * 0.34, H * 0.012, H * 0.02), wiperMat);
      wiper.position.set(wp * cabW * 0.18, hoodY + H * 0.03, frontLowZ + L * 0.015);
      wiper.rotation.y = wp * 0.28; wiper.rotation.x = -0.25;
      glassGrp.add(wiper);
    }
  })();
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    const lampZside = (fit && fit.lampZs) ? Math.min(-L * 0.30, Math.max(-L * 0.5, fit.lampZs[side])) : lampZ;
    ...
    // eyebrow LED accent above the lamp
    const brow = new THREE.Mesh(new THREE.BoxGeometry(lampR * 2.2, lampR * 0.12, 0.04),
      new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xdff0ff, emissiveIntensity: 1.8 }));
    brow.position.set(0, lampR * 1.18, -0.03); brow.rotation.x = -0.15; head.add(brow);
    ...
    // clear glass cover sealing the cluster
    const cover = new THREE.Mesh(new THREE.CircleGeometry(lampR * 1.05, 18),
      new THREE.MeshPhysicalMaterial ? new THREE.MeshPhysicalMaterial({ color: 0xdfe8f5, metalness: 0.1, roughness: 0.03, transparent: true, opacity: 0.25, clearcoat: 1.0, side: THREE.DoubleSide })
        : new THREE.MeshBasicMaterial({ color: 0xdfe8f5, transparent: true, opacity: 0.2 }));
    cover.position.z = -0.09; cover.rotation.y = Math.PI; head.add(cover);
    // amber turn-signal chip beside the projector
    const amber = new THREE.Mesh(new THREE.BoxGeometry(lampR * 0.5, lampR * 0.3, 0.04),
      new THREE.MeshStandardMaterial({ color: 0xffa020, emissive: 0xff9010, emissiveIntensity: 1.2 }));
    amber.position.set(s * lampR * 0.7, lampR * 0.55, -0.05); head.add(amber);
