...
function buildArena(scene) {
  ...
  (function aoStrips() {
    ...
    [[ARENA.W, 0, -HALF_L + 2.5, 0], [ARENA.W, 0, HALF_L - 2.5, Math.PI], [ARENA.L, -HALF_W + 2.5, 0, -Math.PI / 2], [ARENA.L, HALF_W - 2.5, 0, Math.PI / 2]].forEach(d => {
      const strip = new THREE.Mesh(new THREE.PlaneGeometry(d[0], 5), aoMat);
  ...
  // --- curved corner filler panels (rounded-arena feel) ---
  const cornerMat = new THREE.MeshStandardMaterial({ color: 0xbfd4ff, transparent: true, opacity: 0.10, side: THREE.DoubleSide, depthWrite: false });
  [[-1, -1], [1, -1], [-1, 1], [1, 1]].forEach(s => {
    const quarter = new THREE.Mesh(new THREE.CylinderGeometry(10, 10, ARENA.WALL_H, 10, 1, true, 0, Math.PI / 2), cornerMat);
    quarter.position.set(s[0] * (HALF_W - 10), ARENA.WALL_H / 2, s[1] * (HALF_L - 10));
    quarter.rotation.y = Math.atan2(s[1], s[0]) + Math.PI;
    grp.add(quarter);
  });
