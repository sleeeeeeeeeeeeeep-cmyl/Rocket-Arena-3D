...
function buildArena(scene) {
  ...
  const standMat = new THREE.MeshStandardMaterial({ color: 0x121826, roughness: 0.9 });
  for (let t = 0; t < 6; t++) {
    const y = ARENA.WALL_H + 1 + t * 3.4, inset = t * 4.2;
    const longGeo = new THREE.BoxGeometry(ARENA.W + 26 + inset * 2, 2.6, 6);
    [HALF_L + 12 + inset, -(HALF_L + 12 + inset)].forEach(z => { const m = new THREE.Mesh(longGeo, standMat); m.position.set(0, y, z); grp.add(m); });
    const sideGeo = new THREE.BoxGeometry(6, 2.6, ARENA.L + 26 + inset * 2);
    [HALF_W + 12 + inset, -(HALF_W + 12 + inset)].forEach(x => { const m = new THREE.Mesh(sideGeo, standMat); m.position.set(x, y, 0); grp.add(m); });
  }
