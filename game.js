...
(function () {
  ...
  function tintCar(group, color) {
    group.traverse(o => {
      if (o.isMesh && o.material) {
        // clone so we don't tint other instances, then multiply the base color
  ...
  async function startMatch() {
    ...
    if (playerColor !== 0xffffff) tintCar(pGroup, playerColor);
    scene.add(pGroup);
  ...
  function goalScored(team) {
    ...
    // screen flash + GOAL message
    const fl = DOM('flash'); if (fl) { fl.classList.add('go'); setTimeout(() => fl.classList.remove('go'), 140); }
    const cm = DOM('centerMsg'); if (cm) { cm.textContent = 'GOAL!'; cm.style.color = team === 'blue' ? '#46b9ff' : '#ff8a1a'; cm.classList.add('show'); setTimeout(() => cm.classList.remove('show'), 1600); }
