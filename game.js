...
(function () {
  ...
  let matchTime = 300, chosenCarId = 'placeholder', chosenMutator = 'none';
  const MUTATORS = {
  ...
  let camYaw = 0, camPitch = 0.32, dragging = false, lastX = 0, lastY = 0;
  ...
  function makeCar(group, team) {
    return { group, team, pos: new V3(), vel: new V3(), yaw: team === 'blue' ? 0 : Math.PI,
      onGround: true, boost: START_BOOST, boosting: false, wheels: group.userData.wheels || [] };
  }
  ...
  async function startMatch() {
    ...
    score = { blue: 0, orange: 0 }; matchTime = 300;
  ...
  function goalScored(team) {
    ...
    setTimeout(() => { if (ball.mesh) ball.mesh.visible = true; kickoff(); state = 'play'; }, 1800);
  }
  ...
  function updateCamera(dt) {
    ...
    // car forward is (-sin yaw, -cos yaw); chase cam sits BEHIND the car (opposite forward)
    const baseYaw = player.yaw + camYaw;
    const fx = -Math.sin(baseYaw), fz = -Math.cos(baseYaw);
    const dist = 18, height = 8 + camPitch * 7;
    const cx = player.pos.x - fx * dist;   // behind the car
    const cz = player.pos.z - fz * dist;
    camera.position.lerp(new V3(cx, player.pos.y + height, cz), 1 - Math.exp(-7 * dt));
    // look ahead of the car toward the ball
