...
(function () {
  ...
  const ARENA_THEMES = [
    { id: 'night', name: 'Midnight', sky: 0x070b18, turf: 0x1d5230, fog: 0x05080f },
    { id: 'sunset', name: 'Sunset', sky: 0x2a1430, turf: 0x2a5536, fog: 0x281020 },
    { id: 'ice', name: 'Ice', sky: 0x0a2236, turf: 0x3a6a8a, fog: 0x0a1828 },
    { id: 'neon', name: 'Neon', sky: 0x0a061a, turf: 0x1a1040, fog: 0x0a0420 },
    { id: 'day', name: 'Day', sky: 0x6aa0d8, turf: 0x2a7a3e, fog: 0x9ec0e0 }
  ];
  const MUTATORS = {
  ...
  function buildMenu() {
    ...
    const cpk = DOM('colorpick'); if (cpk) { cpk.innerHTML = '';
      ...
      colors.forEach(col => {
        ...
        b.onclick = () => { playerColor = col; [...cpk.children].forEach(x => x.classList.remove('on')); b.classList.add('on'); };
        cpk.appendChild(b);
      });
    }
    const dpk = DOM('decalpick'); if (dpk) { dpk.innerHTML = '';
      DECALS.forEach(dc => {
        const b = document.createElement('button');
        b.className = 'carbtn' + (dc.id === chosenDecal ? ' on' : '');
        b.textContent = dc.name;
        b.onclick = () => { chosenDecal = dc.id; [...dpk.children].forEach(x => x.classList.remove('on')); b.classList.add('on'); };
        dpk.appendChild(b);
      });
    }
  ...
  function styleCar(group, color, decalId) {
    ...
    group.traverse(o => {
      if (o.isMesh && o.material) {
        ...
        if (color !== 0xffffff) o.material.color = new THREE.Color(color);
  ...
  function applyArenaTheme(id) {
    ...
    scene.fog = new THREE.Fog(th.fog, 140, 420);
    if (arenaGrp) arenaGrp.traverse(o => {
      if (o.userData && o.userData.turf) {
        const base = new THREE.Color(th.turf);
        if (o.userData.stripe === 1) base.offsetHSL(0, 0, 0.06);
        else if (o.userData.stripe === 0) base.offsetHSL(0, 0, -0.04);
        o.material.color = base; o.material.needsUpdate = true;
      }
    });
  }
  ...
  async function startMatch() {
    ...
    if (playerColor !== 0xffffff || chosenDecal !== 'none') styleCar(pGroup, playerColor, chosenDecal);
    scene.add(pGroup);
