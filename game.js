...
(function () {
  ...
  function buildMenu() {
    ...
    const acp = DOM('accentpick'); if (acp) { acp.innerHTML = '';
      const acolors = [0xffffff, 0x101318, 0xff3b3b, 0x2f8bff, 0xffd23f, 0x2bd34a, 0xff4ab8, 0x00e5d0, 0xb84aff];
      acolors.forEach(col => {
        const b = document.createElement('button');
        b.className = 'carbtn' + (col === accentColor ? ' on' : '');
        b.style.background = '#' + col.toString(16).padStart(6, '0');
        b.style.width = '34px'; b.style.height = '34px'; b.style.padding = '0';
        b.onclick = () => { accentColor = col; [...acp.children].forEach(x => x.classList.remove('on')); b.classList.add('on'); saveCustomization(); refreshPreview(); };
        acp.appendChild(b);
      });
    }
    ...
    const epk = DOM('explopick'); if (epk) { epk.innerHTML = '';
      EXPLOSIONS.forEach(ex => {
        ...
        b.innerHTML = ex.name + (owned ? '' : ' <span class="price">◉' + ex.price + '</span>');
        b.onclick = () => {
    ...
    const apk = DOM('arenapick'); if (apk) { apk.innerHTML = '';
      ARENA_THEMES.forEach(th => {
        ...
        g.beginPath(); g.ellipse(54, 52, 14, 5, 0, 0, Math.PI * 2); g.stroke();
        // signature props
        ...
        cv.style.cssText = 'width:76px;height:45px;border-radius:7px;display:block';
        b.appendChild(cv);
  ...
  async function startMatch() {
    ...
    score = { blue: 0, orange: 0 }; matchTime = matchLen; overtime = false; otTime = 0;
  ...
  function updateHUD() {
    ...
    if (overtime) { const om = Math.floor(otTime / 60), os = Math.floor(otTime % 60); DOM('timer').textContent = '+OT ' + om + ':' + (os < 10 ? '0' : '') + os; } else { DOM('timer').textContent = m + ':' + (s < 10 ? '0' : '') + s; } DOM('timer').classList.toggle('low', matchTime < 60 && state === 'play');
