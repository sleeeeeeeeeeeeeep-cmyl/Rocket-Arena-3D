...
(function () {
  ...
  function buildMenu() {
    ...
    const epk = DOM('explopick'); if (epk) { epk.innerHTML = '';
      EXPLOSIONS.forEach(ex => {
        ...
        b.className = 'carbtn explobtn' + (ex.id === chosenExplosion ? ' on' : '') + (owned ? '' : ' locked');
        let dots = '';
  ...
  function pvBurst(ex) {
    try {
      if (!pvScene) return;
      for (let i = pvFX.length - 1; i >= 0; i--) { pvScene.remove(pvFX[i]); pvFX.splice(i, 1); }
      const n = 30;
      for (let i = 0; i < n; i++) {
        let colHex;
        ...
        const sp = new THREE.Mesh(new THREE.SphereGeometry(0.16, 8, 6),
        ...
        const spd = (2.2 + Math.random() * 2.6) * Math.min(1.4, ex.speed || 1);
        sp.userData.v = new THREE.Vector3(Math.cos(a) * Math.cos(e2) * spd, Math.sin(e2) * spd + (ex.rise || 8) * 0.18, Math.sin(a) * Math.cos(e2) * spd);
        sp.userData.t = 0; sp.userData.life = 0.9 + Math.random() * 0.4;
        pvScene.add(sp); pvFX.push(sp);
      }
      const ring = new THREE.Mesh(new THREE.RingGeometry(0.2, 0.34, 28),
        new THREE.MeshBasicMaterial({ color: ex.colors && ex.colors !== 'rainbow' ? ex.colors[0] : 0xffffff, transparent: true, opacity: 0.9, side: THREE.DoubleSide }));
      ring.rotation.x = -Math.PI / 2; ring.position.y = 2.2;
      ring.userData.t = 0; ring.userData.life = 0.5; ring.userData.ring = true;
      pvScene.add(ring); pvFX.push(ring);
    } catch (e) {}
  }
  function pvAnimate() {
    ...
    for (let i = pvFX.length - 1; i >= 0; i--) {
      ...
      u.t += pdt;
      if (u.t >= u.life) { pvScene.remove(sp); if (sp.geometry) sp.geometry.dispose(); sp.material.dispose(); pvFX.splice(i, 1); continue; }
      const kk = u.t / u.life;
      if (u.ring) { const s2 = 1 + kk * 9; sp.scale.set(s2, s2, s2); sp.material.opacity = 0.9 * (1 - kk); continue; }
      u.v.y -= 5 * pdt;
      sp.position.addScaledVector(u.v, pdt);
      if (sp.position.y < 0.15) { sp.position.y = 0.15; u.v.y = Math.abs(u.v.y) * 0.4; }
      sp.material.opacity = 1 - kk;
    }
  ...
  function openGarage() {
    ...
    setTimeout(() => pvBurst(EXPLOSIONS.find(e2 => e2.id === chosenExplosion) || EXPLOSIONS[0]), 600);
    if (!pvRAF) pvAnimate();
  }
